...
def init_db():
    ...
    if c.execute('SELECT COUNT(*) n FROM datasets').fetchone()['n']==0:
        now=datetime.now(timezone.utc).isoformat(); c.execute('INSERT INTO datasets(version,name,source_document,record_count,status,created_at,activated_at) VALUES(?,?,?,?,?,?,?)',(1,'Piscipedia Historical Dataset','ICAR_AIEEA_PG_2026_Final_Ranks.pdf',len(RECORDS),'active',now,now))
